...
//reg name
	static void
_setupRegnameName(MetisMessage *message)
{
#ifdef LOG_CHECK
	printf("_setupRegnameName\n\n");
#endif
	MetisTlvExtent extent = metisTlvSkeleton_GetRegname(&message->skeleton);
	if (extent.offset > 0) {

		message->hasRegname = true;
		message->Regname = metisTlvName_Create(&message->messageHead[extent.offset], extent.length);
#ifdef LOG_CHECK
		printf("message->Regname->memoryLength= %d\n", message->Regname->memoryLength);
		for(int i=0; i<message->Regname->memoryLength;i++)
		{
			printf("message->Regname->memory= %x\n",message->Regname->memory[i]);
		}
		...
		message->hasRegname = false;
		message->Regname = NULL;
	}
}

//add name
	static void
_setupReg_addnameName(MetisMessage *message)
{
#ifdef LOG_CHECK
	printf("_setupReg_addnameName\n\n");
#endif
	MetisTlvExtent extent = metisTlvSkeleton_GetReg_Add_name(&message->skeleton);
	if (extent.offset > 0) {

		message->hasReg_addname = true;
		message->Reg_addname = metisTlvName_Create(&message->messageHead[extent.offset], extent.length);
#ifdef LOG_CHECK
		printf("message->Reg_addname->memoryLength= %d\n", message->Reg_addname->memoryLength);
		for(int i=0; i<message->Reg_addname->memoryLength;i++)
		{
			printf("message->Reg_addname->memory= %x\n",message->Reg_addname->memory[i]);
		}
		...
		message->hasReg_addname = false;
		message->Reg_addname = NULL;
	}
}
...
//del name
	static void
_setupReg_delnameName(MetisMessage *message)
{
#ifdef LOG_CHECK
	printf("_setupReg_delnameName\n\n");
#endif
	MetisTlvExtent extent = metisTlvSkeleton_GetReg_Del_name(&message->skeleton);
	if (extent.offset > 0) {

		message->hasReg_delname = true;
		message->Reg_delname = metisTlvName_Create(&message->messageHead[extent.offset], extent.length);
#ifdef LOG_CHECK
		printf("message->Reg_delname->memoryLength= %d\n", message->Reg_delname->memoryLength);
		for(int i=0; i<message->Reg_delname->memoryLength;i++)
		{
			printf("message->Reg_delname->memory= %x\n",message->Reg_delname->memory[i]);
		}
		...
		message->hasReg_delname = false;
		message->Reg_delname = NULL;
	}
}

//dereg name
	static void
_setupReg_deregnameName(MetisMessage *message)
{
#ifdef LOG_CHECK
	printf("_setupReg_deregnameName\n\n");
#endif
	MetisTlvExtent extent = metisTlvSkeleton_GetReg_Dereg_name(&message->skeleton);
	if (extent.offset > 0) {

		message->hasReg_deregname = true;
		message->Reg_deregname = metisTlvName_Create(&message->messageHead[extent.offset], extent.length);
#ifdef LOG_CHECK
		printf("message->Reg_deregname->memoryLength= %d\n", message->Reg_deregname->memoryLength);
		for(int i=0; i<message->Reg_deregname->memoryLength;i++)
		{
			printf("message->Reg_deregname->memory= %x\n",message->Reg_deregname->memory[i]);
		}
		...
		message->hasReg_deregname = false;
		message->Reg_deregname = NULL;
	}
}
...
//registration
	static void
_setupRegistration(MetisMessage *message)
{


	MetisTlvExtent extent = metisTlvSkeleton_GetRegistration(&message->skeleton);
	if (extent.offset !=0 )
	{
		message->registration_from_Consumer = true;
		if(extent.length !=0 )
		{

			message->registration_from_CR = true;
		}
		else
		{
			message->registration_from_CR= false;
		}
	}
	else
	{
		message->registration_from_Consumer= false;
		message->registration_from_CR= false;
	...
	static void
_setupRegistration_add(MetisMessage *message)
{
	MetisTlvExtent extent = metisTlvSkeleton_GetRegistration_add(&message->skeleton);
	if (extent.offset !=0 )
	{
		message->registration_add_from_Consumer = true;
		if(extent.length !=0 )
		{

			message->registration_add_from_CR = true;
		}
		else
		{
			message->registration_add_from_CR= false;
		}
	}
	else
	{
		message->registration_add_from_Consumer= false;
		message->registration_add_from_CR= false;
	...
	static void
_setupRegistration_del(MetisMessage *message)
{
	MetisTlvExtent extent = metisTlvSkeleton_GetRegistration_del(&message->skeleton);
	if (extent.offset !=0 )
	{
		message->registration_del_from_Consumer = true;
		if(extent.length !=0 )
		{

			message->registration_del_from_CR = true;
		}
		else
		{
			message->registration_del_from_CR= false;
		}
	}
	else
	{
		message->registration_del_from_Consumer= false;
		message->registration_del_from_CR= false;
	...
	static void
_setupRegistration_dereg(MetisMessage *message)
{
	MetisTlvExtent extent = metisTlvSkeleton_GetRegistration_dereg(&message->skeleton);
	if (extent.offset !=0 )
	{
		message->registration_dereg_from_Consumer = true;
		if(extent.length !=0 )
		{

			message->registration_dereg_from_CR = true;
		}
		else
		{
			message->registration_dereg_from_CR= false;
		}
	}
	else
	{
		message->registration_dereg_from_Consumer= false;
		message->registration_dereg_from_CR= false;
...
//by wschoi
	static void
_setupKeynameName(MetisMessage *message)
{
#ifdef LOG_CHECK
	printf("_setupKeynameName\n\n");
#endif
	...
	printf("\n\n");
#endif
	MetisTlvExtent extent = metisTlvSkeleton_GetKeyname(&message->skeleton);
#ifdef LOG_CHECK
	printf("extent.offset=%d\n\n", extent.offset);
#endif
	if (extent.offset > 0) {

		message->hasKeyname = true;
		message->Keyname = metisTlvName_Create(&message->messageHead[extent.offset], extent.length);
	} else {
		message->hasKeyname = false;
		message->Keyname = NULL;
	}
}
...
//by wschoi
static void
_setupGetnameName(MetisMessage *message)
{
#ifdef LOG_CHECK
	printf("_setupGetnameName\n\n");
#endif
    MetisTlvExtent extent = metisTlvSkeleton_GetGetname(&message->skeleton);
#ifdef LOG_CHECK
	printf("extent.offset=%d\n\n", extent.offset);
#endif
    if (extent.offset > 0) {

        message->hasGetname = true;
        message->Getname = metisTlvName_Create(&message->messageHead[extent.offset], extent.length);
    } else {
        message->hasGetname = false;
...
//by wschoi
	...
	static void
_setupPayloadGetnameName(MetisMessage *message)
{
#ifdef LOG_CHECK
	printf("_setupPayloadGetnameName\n\n");
#endif
	MetisTlvExtent extent = metisTlvSkeleton_GetPayloadGetname(&message->skeleton);
#ifdef LOG_CHECK
	printf("extent.offset=%d\n\n", extent.offset);
#endif
	if (extent.offset > 0) {

		message->hasPayloadGetname = true;
		message->PayloadGetname = metisTlvName_Create(&message->messageHead[extent.offset], extent.length);
	} else {
		message->hasPayloadGetname = false;
		message->PayloadGetname = NULL;
	}
}
static void
...
_setupInternalData(MetisMessage *message)
{
    ...
    if (goodSkeleton) {
	    //by wschoi
	    _setupRegistration(message);
	    _setupRegistration_add(message);
	    _setupRegistration_del(message);
	    _setupRegistration_dereg(message);

	    _setupRegnameName(message);
	    _setupReg_addnameName(message);
	    _setupReg_delnameName(message);
	    _setupReg_deregnameName(message);
	...
	//by wschoi
	_setupGetnameName(message);
	_setupPayloadGetnameName(message);
	_setupKeynameName(message);
...
//by wschoi
#ifdef LOG_CHECK
	printf("metisMessage_ReadFromBuffer()\n\n");
#endif
...
MetisTlvName *
metisMessage_GetGetname(const MetisMessage *message)
{
    assertNotNull(message, "Parameter message must be non-null");
...
bool
metisMessage_HasGetname(const MetisMessage *message)
{
    assertNotNull(message, "Parameter message must be non-null");
    return message->hasGetname;
}
