...
void ccnxTlvDictionary_SetLookupType(CCNxTlvDictionary *dictionary, uint16_t type)
{
dictionary->LookupType=type;

}

uint16_t ccnxTlvDictionary_CheckLookupType(CCNxTlvDictionary *dictionary)
{

	uint16_t LookupType=dictionary->LookupType;
	if(LookupType  > 111)
	{

#ifdef LOG_CHECK
		printf("ccnxTlvDictionary_CheckLookupType(), LookupType : %x\n", LookupType);
#endif
		return LookupType;
	}
	else
...
PARCBuffer *
ccnxTlvDictionary_GetBuffer_lookup(const CCNxTlvDictionary *dictionary, uint32_t key)
{
    assertNotNull(dictionary, "Parameter dictionary must be non-null");
    assertTrue(key < dictionary->fastArraySize, "Parameter key must be less than %zu", dictionary->fastArraySize);
